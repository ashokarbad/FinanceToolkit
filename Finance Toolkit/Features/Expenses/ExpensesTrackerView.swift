...
private func generateExpensesPDF(
    ...
    let title = NSAttributedString(string: L("Monthly Expenses Report"), attributes: titleAttr)
    ...
    let totalStr = NSAttributedString(string: "\(L("Total")): \(CurrencySettings.formatCurrency(grandTotal, code: currency))", attributes: totalAttr)
    ...
    NSAttributedString(string: L("Category"), attributes: headerAttr).draw(at: CGPoint(x: col1X, y: y))
    ...
    for item in categoryTotals {
        ...
        NSAttributedString(string: CurrencySettings.formatCurrency(item.total, code: currency), attributes: cellBoldAttr)
    ...
    // --- Recent entries ---
    if !entries.isEmpty {
        NSAttributedString(string: L("Recent Entries"), attributes: [
        ...
        NSAttributedString(string: L("Date"), attributes: headerAttr).draw(at: CGPoint(x: dateCol, y: y))
        NSAttributedString(string: L("Category"), attributes: headerAttr).draw(at: CGPoint(x: catCol, y: y))
        NSAttributedString(string: L("Amount"), attributes: headerAttr).draw(at: CGPoint(x: amtCol, y: y))
        y += 16
        ...
        for entry in entries.prefix(20) {
            ...
            NSAttributedString(string: CurrencySettings.formatCurrency(entry.amount, code: currency), attributes: cellBoldAttr)
    ...
    let footer = NSAttributedString(string: L("Generated by Finance Toolkit"), attributes: footerAttr)
...
struct ExpensesTrackerView: View {
    ...
    var body: some View {
        ...
        .alert(L("Delete Expense"), isPresented: $showDeleteAlert) {
            ...
            Text(L("Are you sure you want to remove this expense entry?"))
        }
    }
    ...
    private func shareExpensesCSV() {
        ...
        var csv = "\(L("Date")),\(L("Category")),\(L("Amount")) (\(currency))\n"
        ...
        csv += "\n\(L("Total")),,\(String(format: "%.2f", grandTotal))\n"
    ...
    private var totalCard: some View {
        VStack(spacing: 8) {
            Text(L("Total Expenses"))
                .font(.caption)
                .foregroundStyle(.secondary)
            Text(CurrencySettings.formatCurrency(grandTotal, code: currency))
                .font(.title.bold())
                .foregroundStyle(Color.navy)
                .contentTransition(.numericText())
            Text("\(currentMonthExpenses.count) \(L("entries this month"))")
                .font(.caption2)
            ...
            if currentMonthExpenses.isEmpty {
                VStack(spacing: 10) {
                    Image(systemName: "tray")
                        ...
                        .foregroundStyle(.tertiary)
                    Text(L("No expenses recorded this month"))
                        .font(.caption)
                        .foregroundStyle(.secondary)
                    Button(L("Add Expense")) { showAddSheet = true }
                        .font(.caption.weight(.semibold))
                        .foregroundStyle(Color.navy)
                }
                .frame(maxWidth: .infinity)
                ...
                ForEach(currentMonthExpenses.prefix(10)) { entry in
                    ...
                    HStack(spacing: 12) {
                        ...
                        Spacer()
                        Text(CurrencySettings.formatCurrency(entry.amount, code: currency))
                            .font(.subheadline.weight(.semibold))
...
struct YearlyExpensesOverview: View {
    ...
    var body: some View {
        NavigationStack {
                    ...
                    if monthlyData.isEmpty {
                        VStack(spacing: 14) {
                            Image(systemName: "chart.bar")
                                ...
                                .foregroundStyle(.tertiary)
                            Text(L("No expense data for") + " \(year.description)")
                                .font(.headline)
                                .foregroundStyle(.secondary)
                            Text(L("Add expenses to see your yearly overview here."))
                                .font(.subheadline)
                                .foregroundStyle(.tertiary)
            ...
            .navigationTitle(L("Yearly Expenses") + " \(year.description)")
...
struct AddExpenseSheet: View {
    ...
    var body: some View {
        NavigationStack {
            Form {
                Section {
                    SectionHeader(systemImage: isEditMode ? "pencil.circle.fill" : "plus.circle.fill", title: isEditMode ? L("Edit Expense") : L("New Expense"), color: .navy)
                }
                ...
                Section(L("Category")) {
                    ...
                    if selectedCategory == .other {
                        HStack {
                            Text(L("Name"))
                            Spacer()
                            TextField(L("e.g. Donation, Gift"), text: $customCategoryName)
                ...
                Section {
                    Button {
                        ...
                        HStack {
                            Spacer()
                            Label(isEditMode ? L("Update Expense") : L("Add Expense"), systemImage: isEditMode ? "checkmark.circle.fill" : "plus.circle.fill")
                                .font(.headline)
                            Spacer()
                        }
                        .padding(.vertical, 6)
            ...
            .keyboardDoneToolbar()
            .navigationTitle(isEditMode ? L("Edit Expense") : L("Add Expense"))
            .navigationBarTitleDisplayMode(.inline)
            .toolbar {
                ToolbarItem(placement: .topBarLeading) {
...
struct DatePickerSheet: View {
    ...
    var body: some View {
        NavigationStack {
            DatePicker(L("Select Date"), selection: $selectedDate, displayedComponents: .date)
